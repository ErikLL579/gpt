...
Uses corrected Fourier kernel (proper P^L projector normalization).
"""
import gpt as g
import numpy as np
import random
...
g.message("Starting Fourier HMC with 50/50 gauge fixing (corrected kernel)...")
...
for it in range(n_trajectories):
    ...
    g.message(f"[{it}/{n_trajectories}] plaq = {plaq:.6f}, dH = {dH:.6e}, acc = {accept/total:.3f}")

# Save measurements
np.save("plaq_measurements_fourier_gf_test.npy", np.asarray(plaq_measurements))
np.save("trace_measurements_fourier_gf_test.npy", np.asarray(trace_measurements))
np.save("wf_measurements_fourier_gf_test.npy", np.asarray(wf_measurements))

g.message(f"Final acceptance rate: {accept/total:.3f}")
g.message("Measurements saved.")
