...
class local_gtf(local_diffeomorphism):
    @params_convention(dimension=None, checkerboard=None, rho=None, staple_field=None)
    def __init__(self, params):
        self.params = params
        self.cache = {}
        self.verbose = g.default.is_verbose("GTF_performance")

    def get_C(self, fields):
        grid = fields[0].grid
        nd = grid.nd
        U = fields[0:nd]
        rho = 0.05 # tune for gtf in future
        ...
        if False:
            ...
            #        continue
                #st += rho * g.qcd.gauge.staple(U, self.params["dimension"], nu) # need to modify to be dA instead
            st = rho * div_A(U)
    ...
    def jacobian(self, fields, fields_prime, src):
        ...
        #
        # Sigma == g.adj(U) * gradient * 1j ### grad S(U)
        #
