#!/usr/bin/env python3
#
# Nambu HMC with 5D OBC scaffold in G, with G LINEAR in r (arXiv:2409.18958
# eq. 30 form): production run for the topological tunneling comparison
# (counterpart of hmc_topo_run.py and nambu_obc5d_topo_run.py, same
# measurements and log format; analysis: tau_int.py).
#
...
# gamma = 1, kappa5 = 0 reduces exactly to standard HMC.
#
...
seed = g.default.get("--seed", "nambu-obc5d-linear-topo")
...
g.message(
    f"""
Nambu HMC with 5D OBC scaffold (G linear in r): topological tunneling run
  lattice = {grid.fdimensions}
  beta    = {beta}
  gamma   = {gamma}, kappa5 = {kappa5}
