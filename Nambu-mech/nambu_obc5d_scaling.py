...
#
# Forces for all 9 link species via one reverse-mode AD functional;
# e_a H = 0 for V, W so their forces come purely from the G-coupling.
#
...
seed = g.default.get("--seed", "nambu-obc5d-test")
...
g.message(
    f"""
Nambu HMC with 5D OBC scaffold in G: scaling test
...
def hamiltonian_G():
    return c_p * a_kin(Pbig) + c_r * a_kin(Rbig) + kappa5 * s5_func(Ubig)
...
################################################################################
g.message("=" * 70)
g.message("Test 1: reversibility of PRURP with 5D OBC scaffold")
