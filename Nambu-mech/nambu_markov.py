#!/usr/bin/env python3
#
# Nambu HMC Markov chain (arXiv:2409.18958)
#
# Full algorithm: PRURP trajectories with Metropolis accept/reject on
#   H(p,U,r) = sum p^2/2 + sum r^2/2 + S_beta(U)
# with auxiliary Hamiltonian
#   G(p,U,r) = c_p sum p^2/2 + c_r sum r^2/2 + S_{lam*beta}(U).
# p and r are drawn fresh from a unit Gaussian at the start of each
# trajectory; on reject, U is restored (p, r are redrawn anyway).
#
# Reports plaquette history, acceptance, <|dH|>, and wall-clock timing.
#
import gpt as g
import numpy as np
import time
...
def frc_P():
    F = a_S.gradient(U, U)
    return scaled_list(c_r - lam, cw_list(F, R))


def frc_R():
    F = a_S.gradient(U, U)
    return scaled_list(lam - c_p, cw_list(F, P))


def vel_U():
    return scaled_list(c_r - c_p, cw_list(P, R))
